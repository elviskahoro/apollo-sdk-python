...
__all__ = ["TaskBulkCreateParams"]


class TaskBulkCreateParams(TypedDict, total=False):
    contact_ids: Required[SequenceNotStr[str]]
    """
    The Apollo IDs for the contacts that you want to be on the receiving end of the
    action.
    ...
    <li> `call`: Call the contacts. </li> <li> `outreach_manual_email`: Email the
    contacts. </li> <li> `linkedin_step_connect`: Send a LinkedIn invitation to
    connect with the contacts. </li> <li> `linkedin_step_message`: Send a direct
    message to the contacts' LinkedIn profiles. </li> <li>
    `linkedin_step_view_profile`: View the contacts' LinkedIn profiles. </li> <li>
    `linkedin_step_interact_post`: Interact with the contacts' recent LinkedIn
    posts. </li> <li> `action_item`: Take generic action for the contacts. If you
    use this task type, Apollo recommends using the `note` parameter too. </li>
    </ul>
    """
