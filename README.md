# Apollo SDK Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/apollo_sdk.svg?label=pypi%20(stable))](https://pypi.org/project/apollo_sdk/)

The Apollo SDK Python library provides convenient access to the Apollo SDK REST API from any Python 3.9+
...
pip install git+ssh://git@github.com/stainless-sdks/apollo-sdk-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install apollo_sdk`
...
Simply import `AsyncApolloSDK` instead of `ApolloSDK` and use `await` with each API call:
...
```sh
# install from this staging repo
pip install 'apollo_sdk[aiohttp] @ git+ssh://git@github.com/stainless-sdks/apollo-sdk-python.git'
```
...
You can enable logging by setting the environment variable `APOLLO_SDK_LOG` to `info`.

```shell
$ export APOLLO_SDK_LOG=info
```
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/apollo-sdk-python/issues) with questions, bugs, or suggestions.
